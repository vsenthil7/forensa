...
# Start npm run dev with the freshly-minted env captured.
$envForDev = @{
...
$envArgs = ($envForDev.GetEnumerator() | ForEach-Object { "`$env:$($_.Key)='$($_.Value)'" }) -join '; '
# pnpm dev script: see apps/console/package.json
$devCmd = "cd '$frontendDir'; $envArgs; npm run dev"
