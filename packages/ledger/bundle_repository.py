...
async def get_active_bundle_for_tenant(
    ...
    """Return the most recently created ``PolicyBundle`` for the tenant.

    Today "active" means "most recently created" - there is no per-bundle
    activation/approval state yet. When the approval workflow lands
    (``NEW-P9.8.X.bundle-approval-workflow``) this function will filter on a
    new ``status='active'`` column. Until then, callers (notably the
    ``PostgresBundleProvider`` in ``apps.api.ingest_service``) can rely on
    "latest written = active".

    Returns ``None`` if the tenant has no bundles yet - the ``DefaultBundleProvider``
    fallback path in the ingest service can then issue a default bundle.
    """
    stmt = (
